...
async function insertReviews(connection, reviews, placeName) {
    try {
        ...
        console.log(`Deleted ${deleteResult.affectedRows} rows from reviews table.`);
    ...
    try {
        ...
        const reviews = await page.evaluate(() => {
            ...
            reviewElements.forEach((reviewElement) => {
                const reviewerName = reviewElement.querySelector('.d4r55') ? reviewElement.querySelector('.d4r55').innerText : 'No name';
                const reviewText = reviewElement.querySelector('.wiI7pd') ? reviewElement.querySelector('.wiI7pd').innerText : 'No review text';
        ...
        await browser.close();
        return { success: true, message: 'All reviews scraped and saved successfully.' };
    } catch (error) {
        console.error('Error during scraping:', error);
        return { success: false, message: 'Error occurred while scraping reviews.' };
    } finally {
        if (connection) {
